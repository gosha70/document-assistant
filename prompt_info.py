...
class PromptInfo:
    ...
    def __str__(self):
        return (
            f"PromptInfo(system_prompt='{self._system_prompt}', "
    ...
    def get_prompt_template(self):
        if self._template_type == LLAMA_MODEL_NAME:  # https://huggingface.co/blog/llama2#how-to-prompt-llama-2
            ...
            if self._use_history:
                ...
                prompt_template = (
                    INSTRUCTION_START + self._system_prompt + "\nContext: {context}\nUser: {question}" + INSTRUCTION_END
                )
        else:
            if self._use_history:
        ...
        if self._use_history:
            # LangChain 1.x removed ConversationBufferMemory; use langchain-classic.
            # Override showwarning to suppress LangChainDeprecationWarning which
            # bypasses standard category filters due to stacklevel manipulation.
